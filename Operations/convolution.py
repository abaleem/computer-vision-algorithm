...
def conv(image, kernel, padding=True):
    ...
    # rotates the kernel by 180 degrees
    ...
    for channel in range(image_output.shape[2]):
        for height in range(image_output.shape[0]):
            for width in range(image_output.shape[1]):
                ...
                if (round(np.sum(np.multiply(image_unit, kernel))) < 0):
                    pass
                    #print("negative value", round(np.sum(np.multiply(image_unit, kernel))))
                    #print("image_unit:", image_unit)
                    #print("kernel:", kernel)
